...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: carrito.php?error=metodo_no_permitido');
    exit;
}

if (!isset($_POST['producto_id']) || !isset($_POST['cantidad'])) {
    header('Location: carrito.php?error=datos_faltantes');
    exit;
}
...
if (isset($_SESSION['carrito'][$producto_id])) {
    ...
    header('Location: carrito.php?success=cantidad_actualizada');
} else {
    // Si el producto no está en el carrito, añadirlo
    $_SESSION['carrito'][$producto_id] = ['cantidad' => $cantidad];
    header('Location: carrito.php?success=producto_anadido');
}
