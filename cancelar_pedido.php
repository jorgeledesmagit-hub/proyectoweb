...
try {
    ...
    $stmt = $db->prepare("SELECT id, estado FROM ordenes WHERE id = ? AND usuario_id = ? FOR UPDATE");
    ...
    $stmt = $db->prepare("SELECT producto_id, cantidad FROM detalles_orden WHERE pedido_id = ?");
